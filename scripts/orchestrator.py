...
SEM_GATE          = float(os.getenv("SEM_GATE",          "0.65"))
MIN_SENT_SIM_GATE = float(os.getenv("MIN_SENT_SIM_GATE", "0.35"))
LEN_RATIO_MIN     = float(os.getenv("LEN_RATIO_MIN",     "0.60"))
LEN_RATIO_MAX     = float(os.getenv("LEN_RATIO_MAX",     "1.80"))

# Utility weights
W_AI      = float(os.getenv("W_AI",      "0.60"))
W_SEM     = float(os.getenv("W_SEM",     "0.35"))
W_RISK    = float(os.getenv("W_RISK",    "0.05"))
...
async def run_pipeline(
    ...
    original_ai = await asyncio.to_thread(mr.run_desklib, text)
    ...
    final_ai  = await asyncio.to_thread(mr.run_desklib,          output_text)
    final_sem = await asyncio.to_thread(mr.run_mpnet_similarity, text, output_text)
...
async def _process_chunk(
    ...
    await broadcast(
        {
            "type": "chunk_start",
            "data": {"chunk_idx": chunk_idx, "total": total_chunks, "text": chunk[:300]},
        }
    )
    ...
    await emit_stage("rewrite", "active", "Generating 3 ensemble candidates")
    candidates_raw: List[Dict[str, Any]] = await asyncio.to_thread(
        mr.run_rewrite_ensemble, chunk
    )
